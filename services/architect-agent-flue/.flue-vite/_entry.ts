...
import * as handler_architect_0 from '/data/data/com.termux/files/home/workspace/ai-drakon-scaffolder/services/architect-agent-flue/src/agents/architect.ts';
import * as workflow_pipeline_a_0 from '/data/data/com.termux/files/home/workspace/ai-drakon-scaffolder/services/architect-agent-flue/src/workflows/pipeline-a.ts';
import * as workflow_pipeline_b_1 from '/data/data/com.termux/files/home/workspace/ai-drakon-scaffolder/services/architect-agent-flue/src/workflows/pipeline-b.ts';
...
const agentModules = {
  "architect": handler_architect_0,
};
const workflowModules = {
  "pipeline-a": workflow_pipeline_a_0,
  "pipeline-b": workflow_pipeline_b_1,
};
...
const workflowClassNames = {
  "pipeline-a": "PipelineAWorkflow",
  "pipeline-b": "PipelineBWorkflow",
};
...
export class Architect extends Agent {
  async onRequest(request) {
    return dispatchAgent(request, this, "architect", directHandlers["architect"]);
  }

  async fetch(request) {
    if (isWebSocketUpgrade(request)) {
      await this.__unsafe_ensureInitialized();
      return acceptAgentSocket(request, this, "architect");
    }
    return super.fetch(request);
  }

  async webSocketMessage(socket, message) {
    if (isFlueSocket(socket, 'agent', "architect")) {
      await this.__unsafe_ensureInitialized();
      return messageAgentSocket(socket, message, this, "architect");
    }
    return super.webSocketMessage(socket, message);
  }

  async webSocketClose(socket, code, reason, wasClean) {
    if (isFlueSocket(socket, 'agent', "architect")) return closeFlueSocket(socket, code, reason);
    return super.webSocketClose(socket, code, reason, wasClean);
  }

  async webSocketError(socket, error) {
    if (isFlueSocket(socket, 'agent', "architect")) return closeFlueSocket(socket, 1011, 'WebSocket error');
    return super.webSocketError(socket, error);
  }

  async onFiberRecovered(ctx) {
    if (ctx.name === 'flue:dispatch') {
      return handleFlueDispatchRecovered(ctx, this, "architect");
    }
    if (ctx.name === 'flue:direct') {
      return handleFlueDirectRecovered(ctx, this, "architect");
    }
...
export class PipelineAWorkflow extends Agent {
  async onRequest(request) {
    return dispatchWorkflow(request, this, "pipeline-a");
  }

  async fetch(request) {
    if (isWebSocketUpgrade(request)) {
      await this.__unsafe_ensureInitialized();
      return acceptWorkflowSocket(request, this, "pipeline-a");
    }
    return super.fetch(request);
  }

  async webSocketMessage(socket, message) {
    if (isFlueSocket(socket, 'workflow', "pipeline-a")) {
      await this.__unsafe_ensureInitialized();
      return messageWorkflowSocket(socket, message, this, "pipeline-a");
    }
    return super.webSocketMessage(socket, message);
  }

  async webSocketClose(socket, code, reason, wasClean) {
    if (isFlueSocket(socket, 'workflow', "pipeline-a")) return closeFlueSocket(socket, code, reason);
    return super.webSocketClose(socket, code, reason, wasClean);
  }

  async webSocketError(socket, error) {
    if (isFlueSocket(socket, 'workflow', "pipeline-a")) return closeFlueSocket(socket, 1011, 'WebSocket error');
    return super.webSocketError(socket, error);
  }

  async onFiberRecovered(ctx) {
    if (ctx.name?.startsWith('flue:workflow:')) {
      return handleFlueWorkflowFiberRecovered(ctx, this, "pipeline-a");
    }
    if (typeof super.onFiberRecovered === 'function') {
      return super.onFiberRecovered(ctx);
    }
...
export class PipelineBWorkflow extends Agent {
  async onRequest(request) {
    return dispatchWorkflow(request, this, "pipeline-b");
  }

  async fetch(request) {
    if (isWebSocketUpgrade(request)) {
      await this.__unsafe_ensureInitialized();
      return acceptWorkflowSocket(request, this, "pipeline-b");
    }
    return super.fetch(request);
  }

  async webSocketMessage(socket, message) {
    if (isFlueSocket(socket, 'workflow', "pipeline-b")) {
      await this.__unsafe_ensureInitialized();
      return messageWorkflowSocket(socket, message, this, "pipeline-b");
    }
    return super.webSocketMessage(socket, message);
  }

  async webSocketClose(socket, code, reason, wasClean) {
    if (isFlueSocket(socket, 'workflow', "pipeline-b")) return closeFlueSocket(socket, code, reason);
    return super.webSocketClose(socket, code, reason, wasClean);
  }

  async webSocketError(socket, error) {
    if (isFlueSocket(socket, 'workflow', "pipeline-b")) return closeFlueSocket(socket, 1011, 'WebSocket error');
    return super.webSocketError(socket, error);
  }

  async onFiberRecovered(ctx) {
    if (ctx.name?.startsWith('flue:workflow:')) {
      return handleFlueWorkflowFiberRecovered(ctx, this, "pipeline-b");
    }
    if (typeof super.onFiberRecovered === 'function') {
      return super.onFiberRecovered(ctx);
    }
