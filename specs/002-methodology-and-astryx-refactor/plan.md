...
**Факт перевірки (2026-08-18):** `/sync` (`src/routes/sync.tsx`) — зареєстрована сторінка-заглушка, не пункт sidebar-навігації; додатково на неї веде `CommandPalette`. Вона не робить fetch до backend і показує лише статус «ще не реалізована». У Worker є тільки окремий WebSocket-контракт `/v1/diagram/:diagramId/sync` для realtime-синхронізації відкритої діаграми — backend-функції синхронізації репозиторію для `/sync` немає.

**Рішення:** залишити `/sync` як явний `planned`/Coming soon маршрут. Це найменш ризикований варіант: він не ламає прямі URL, Command Palette та майбутню сумісність, а текст сторінки більше не обіцяє конкретний «наступний реліз».

`architect-agent-flue`, `drakon-agent-flue` і `docs-agent-flue` реально існують. Вони не є dead WIP: Worker має service bindings для `architect-agent-flue`/`docs-agent-flue`, код використовує URL усіх трьох, а `architect-agent-flue` і `drakon-agent-flue` мають значний незакомічений TS/Wrangler WIP (у stash snapshot від 2026-08-18); останні звичайні commits, що торкалися сервісів, — 2026-06-21 (`architect`/`docs`) і 2026-06-18 (`drakon`). Тому запис про `known dead WIP` у debt policy та видалення сервісів не застосовуються; зафіксовано активний ризик WIP за чинною §2 політикою.

**Зроблено:** уточнено Coming soon UI у `src/routes/sync.tsx` і синхронізовано його з `.lovable/src/`; цей D-3 оновлено фактами та прийнятим рішенням. `routeTree.gen.ts` не змінювався, оскільки маршрут свідомо збережено.
