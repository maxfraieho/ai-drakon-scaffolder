...
- **T-320: ATTEMPTED then REVERTED (2026-08-19, AGY .234 + Claude review).** First attempt removed the `hydrated` gate entirely from both `index.tsx` and `__root.tsx`, replacing it with an unconditional `hasClientJwt()`/`isAuthenticated` check on every render (including the very first client render, pre-hydration). This deletes the exact safeguard a prior code comment documented as fixing a known bug ("infinite /login <-> /diagrams loop" for JWT-only logins) without demonstrating the replacement avoids reintroducing it — `hasClientJwt()` reads `localStorage`, which is unavailable during SSR and can differ between the server-rendered HTML and the client's first paint, risking a hydration mismatch (ironically the same class of flash this task was meant to fix). Reverted `src/lib/route-auth.ts`, `src/routes/index.tsx`, `src/routes/__root.tsx` to pre-batch state — not committed. STILL OPEN: needs a real fix that keeps hydration-safety (e.g. resolve auth state once in a route loader that runs identically server+client, or keep a SHARED single `hydrated` flag consumed by both files instead of removing it) — requires live browser testing before landing, not safe for unattended overnight execution.
