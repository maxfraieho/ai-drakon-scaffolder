...
## Паралельне виконання (AGY + AGY3)

Для оптимізації швидкості розробки та ефективного розподілу квот підтримується паралельне виконання завдань на кількох пристроях:
- **AGY phone (192.168.3.25)**: основний мобільний пристрій розробника, має динамічну IP-адресу та змінну квоту.
- **AGY3 tablet (192.168.3.162)**: виділений планшет для автоматизації, має статичну IP-адресу та 100% доступну квоту.
  - SSH-доступ: `sshpass -p 'TermuxSsh2026!' ssh -o StrictHostKeyChecking=no -p 8022 u0_a410@192.168.3.162`
  - Потребує: розгорнутого `agy-task.sh`, налаштованого Git та клонованого репозиторію `ai-drakon-scaffolder`.

Такий розподіл дозволяє оркестратору паралельно делегувати незалежні підзадачі на різні фізичні пристрої, суттєво зменшуючи загальний час виконання спринтів.

## Локальність MemPalace (MemPalace Locality)

Критично важливо розуміти фізичне розташування та межі роботи бази даних MemPalace:
1. **Локальний характер БД**: MemPalace працює як локальний процес на кожному окремому пристрої (або всередині Termux на телефонах/планшетах, або на OrangePi як MCP-сервер).
2. **Відсутність центрального сервера MemPalace**:
   - Dev-сервер `192.168.3.184` **НЕ має** встановленого MemPalace.
   - Агенти-виконавці (AGY) ніколи не повинні намагатися виконувати SSH-команди до `192.168.3.184` для запису щоденника чи виклику `mempalace`.
   - Замість цього всі операції з `mempalace` (наприклад, `python3 -m mempalace diary write`) на Termux мають запускатися **ЛОКАЛЬНО** на самому пристрої виконавця.
3. **Синхронізація через Щоденник (Diary)**: хоча кожен пристрій має власну відокремлену локальну БД ChromaDB, синхронізація контексту між ними відбувається за рахунок текстового логування щоденника (diary) у файли, які згодом зчитуються та індексуються іншими агентами в процесі `mempalace mine`.
