...
export async function resolveDiagramContext(nameOrId: string): Promise<DiagramWithContext | null> {
...
export function findDiagramsByFilePath(filePath: string, diagrams: Diagram[]): Diagram[] {
  ...
  return diagrams.filter((diagram) =>
    (diagram.diagram.metadata?.filePaths ?? []).some((path) => path.toLowerCase().includes(needle)),
  );
}