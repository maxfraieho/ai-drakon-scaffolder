...
function filterByDepth(nodes: GraphNode[], edges: GraphEdge[], depth: number): { nodes:
...
if (focusedNode) {
const inNeighborhood = focusNeighbors!.has(edge.source) &&
focusNeighbors!.has(edge.target);
...
<Button variant="ghost" size="icon" className="h-8 w-8 ml-1" onClick={() => setViewState({
zoom: 1, panX: 0, panY: 0 })} title="Скинути вигляд">
...
setSearchOpen(true)} title="Пошук вузлів">
...
<Slider value={[depth]} onValueChange={([v]) => setDepth(v)} min={1} max={10}
step={1} className="w-24" />
<span className="text-xs font-medium text-primary w-5 text-center">{depth >= 10 ? '∞' :
depth}</span>
</div>

<div className="flex items-center gap-1 border-l border-border pl-3">
<span className="text-xs text-muted-foreground mr-1">Зв'язки</span>
{([['all', 'Всі'], ['structural-semantic', 'S+S'], ['structural', 'Str']] as [EdgeFilter,
...
{[0, 1, 2, 3].map(lvl => (
<Button key={lvl} variant={expandLevel === lvl ? 'default' : 'ghost'} size="sm"
className="h-6 w-6 p-0 text-xs" onClick={() => setExpandLevel(lvl)}>
{lvl}
</Button>
))}
<Button variant={isolated ? 'default' : 'outline'} size="sm" className="h-7 px-2 text-xs gap-1"
onClick={handleIsolate}>
<Focus className="h-3 w-3" />
Ізолювати
</Button>
<Button variant="ghost" size="icon" className="h-7 w-7" onClick={clearFocus}
title="Скинути фокус">
...
const showLabel = isHovered || isFocused || (hasFocus && highlighted) || viewState.zoom >=
0.8;
...
<span className="text-foreground font-medium">{slugMap.get(focusedNode)?.title ||
focusedNode}</span>
<span>вх: {focusStats.inbound}</span>
<span>вих: {focusStats.outbound}</span>
<span className="text-muted-foreground/60">подвійний клік — відкрити · фон — скинути
фокус</span>
</div>
)}
</div>
