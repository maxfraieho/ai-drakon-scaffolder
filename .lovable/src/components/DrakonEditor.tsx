...
import type {
  DrakonDiagram,
  DrakonWidget as DrakonWidgetType,
  DrakonEditSender,
  DrakonConfig,
...
export function DrakonEditor({
  ...
  const [contextMenu, setContextMenu] = useState<{
    x: number;
    y: number;
    items: Array<{ text: string; action?: () => void; type?: string }>;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      /* ignore if no diagram loaded yet */
    }
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-hidden">
        ...
        <div className="w-full flex-shrink-0 overflow-x-auto border rounded-lg bg-background">
          ...
          <FormatInspector
            ...
            onConfirm={(newStyle: Record<string, unknown>) => {
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {t.drakonEditor.savedIn} {folderSlug || "diagrams"}/diagrams/{diagramId || "id"}
                .drakon.json
              </p>
            </div>
