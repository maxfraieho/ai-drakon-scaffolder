...
// Update domain model endpoint (TASK-DRK-7)
app.post('/docs/domain', async (c) => {
  const req: any = await c.req.json().catch(() => ({}));
  const interview = req.interview || '';
  const project = req.project || 'uav-watcher';
  const slug = req.slug || 'domain';
  const tagList: string[] = req.tags || ['domain', project];
  
  if (!interview) {
    return c.json({ success: false, error: 'interview text is required' }, 400);
  }
  
  const dateStr = new Date().toISOString().split('T')[0];
  
  // 1. Try to read the existing domain.md to update it
  let existingContent = '';
  try {
    const readRes = await notesCrud.execute({
      operation: 'read',
      slug,
      project
    }, { env: c.env } as any);
    const readJson = JSON.parse(readRes);
    if (readJson.success && readJson.content) {
      existingContent = readJson.content;
    }
  } catch (e) {
  ...
  let message = '';
  if (existingContent) {
    message = `Онови або доповни наявний документ доменної моделі (domain model) для проекту ${project} на основі нової інформації з інтерв'ю.
    
Інтерв'ю / Нові вимоги:
"""
${interview}
"""

Поточний вміст docs/${project}/${slug}.md:
"""
${existingContent}
"""

УВАГА: Збережи формат Obsidian Markdown. НЕ обгортай відповідь у \`\`\`markdown або \`\`\`yaml блоки. Починай відразу з --- frontmatter. Якщо потрібно, онови frontmatter (наприклад, додавши нові tags або оновивши title). Збережи структуру розділів:
# Доменна модель: <Назва проекту>
## Опис системи
## Терміни предметної області (Глосарій)
## Бізнес-правила та обмеження
## Запланована архітектура (Модулі)`;
  } else {
    message = `Проаналізуй інтерв'ю або вимоги користувача і створи новий документ доменної моделі (domain model) для проекту ${project}.
...
Документ повинен містити такі секції (УКРАЇНСЬКОЮ мовою):
# Доменна модель: <Назва проекту>
## Опис системи
<Короткий опис системи, її цілей та призначення>

## Терміни предметної області (Глосарій)
<Визначення ключових термінів, концептів та сутностей системи>

## Бізнес-правила та обмеження
<Специфічні правила, логіка перевірок, обмеження системи>

## Запланована архітектура (Модулі)
<Перелік запланованих модулів, які будуть розроблені, з описом їх призначення>

Вхідні вимоги з інтерв'ю:
"""
${interview}
"""`;
  }
