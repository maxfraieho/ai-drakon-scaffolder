  ...
  if (node.type === "dir") {
    return (
      <div className="w-full">
        <div className="group flex items-center hover:bg-white/5 rounded transition-colors pr-2">
          <button
            type="button"
            onClick={() => onToggleFolder(node)}
            className="flex-1 flex items-center gap-1.5 py-1 text-left text-xs text-[var(--text-secondary)] min-w-0"
...
export function ProjectFileManager({ defaultMode = "all" }: ProjectFileManagerProps) {
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-[var(--bg-base)]">
      {/* Collapsible Sidebar */}
      <aside className={cn(
        "shrink-0 flex flex-col border-r border-[var(--border-subtle)] bg-[var(--bg-surface)]/60 backdrop-blur-md transition-[width] duration-200 overflow-hidden",
          ...
          <button
            ...
            className={cn(
              ...
              mode === "all"
                ...
                : "border-transparent text-[var(--text-muted)] hover:text-[var(--text-secondary)] disabled:opacity-30"
            )}
          >
            All
          </button>
      ...
      {!sidebarOpen && (
        <button
          ...
          className="h-full w-6 shrink-0 flex items-center justify-center border-r border-[var(--border-subtle)] bg-[var(--bg-surface)]/20 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors"
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[var(--bg-base)]">
        {/* Center Toolbar */}
        <div className="h-10 px-3 border-b border-[var(--border-subtle)] bg-[var(--bg-surface)]/80 backdrop-blur shrink-0 flex items-center justify-between">
          {/* Breadcrumbs */}
          <div className="font-mono text-[10px] text-[var(--text-muted)] truncate flex items-center gap-1.5">
            ...
            {filePath && (
              <>
                ...
                <span className="text-[var(--text-secondary)] font-bold">{filePath}</span>
              </>
            )}
          </div>
        ...
        <div className="flex-1 min-h-0 relative">
          {filePath ? (
            <div className="w-full h-full flex flex-col">
              {loadingFile && (
                <div className="absolute inset-0 bg-[var(--bg-surface)]/80 backdrop-blur-sm z-10 flex items-center justify-center">
